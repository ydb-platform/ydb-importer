...
/** Source table metadata retrieval - SAP HANA specifics. */
public class HanaTableLister extends AnyTableLister {
    ...
    /*
     * HANA JDBC driver reports TIMESTAMP with scale=0, which would map to
     * Datetime64 and lose fractional seconds. Tag TIMESTAMP columns with
     * scale=7 so the mapper picks Timestamp64.
     */
    @Override
    ...
    /**
     * Retrieve the real PRIMARY KEY definition, if it exists. HANA PK columns
     * are implicitly NOT NULL, so no extra nullability check is needed.
     */
     ...
     * <p>
     * SYS.CONSTRAINTS only contains constraints declared via CREATE TABLE /
     * ALTER TABLE ADD CONSTRAINT, so expression-based and partial UNIQUE
     * indexes (which live in SYS.INDEXES only) are naturally excluded.
     */
    ...
    public List<PartitionInfo> listPartitions(Connection con, TableDecision td, TableMetadata tm)
        ...
        // SYS.TABLE_PARTITIONS has one row per physical partition with PART_ID>0.
        // Row-store tables cannot be partitioned, so the result is empty and
        // the fallback below returns an empty list (single-chunk full scan).
        try (PreparedStatement ps = con.prepareStatement(
