...
public class TableRef extends JdomHelper implements TableIdentity {
    ...
    public TableRef(Element c, Map<String, TableOptions> optionsMap) {
        ...
        if (this.splitBy != null && this.splitCount == null) {
    ...
    private void validateSplit(Element c) {
        boolean anySplit = splitFrom != null || splitTo != null || splitCount != null;
        if (anySplit && splitBy == null) {
            throw raise(c, "split-by is required when any split-* tag is set");
        }
        if ((splitFrom != null) != (splitTo != null)) {
            throw raise(c, "split-from and split-to must both be explicit, or both omitted/auto");
        }
    }

    private void validateYdbPartition(Element c) {
        if ((ydbPartitionFrom != null) != (ydbPartitionTo != null)) {
            throw raise(c, "ydb-partition-from and ydb-partition-to must both be explicit, or both omitted/auto");
        }
    }
