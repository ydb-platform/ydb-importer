...
- **Параллельное чтение источника** через `<split-by>` и `<split-count>`, либо через чтение нативных партиций источника (`<use-source-partitions>`). Задаёт, на сколько частей разбить запросы к источнику.

Оба режима решают три задачи:
- ускорение чтения и записи за счёт параллельных задач,
- независимая обработка ошибок (если задача упала, повторяется только она, см. `<retry-count>`),
- локальность записи, чтобы значения одного батча попадали в одну партицию YDB.

### Партиции источника

Если партиции источника не пересекаются по первой колонке ключа (источник партиционирован по тому же ключу, что и YDB), каждая задача чтения соответствует одной партиции YDB. Это самый эффективный режим.

Если партиции источника пересекаются по первой колонке ключа (источник партиционирован по другому ключу), то без `<split-by>` каждая партиция источника читается одним запросом, но значения попадают в разные партиции YDB при записи батча. Можно дополнительно задать `<split-by>`. В этом случае для каждой партиции источника генерируются подзапросы с фильтром по диапазону split-колонки. Каждая задача соответствует одной партиции YDB.
...
<ydb-importer>
    <workers>
        <!-- Количество потоков чтения (целое число от 1 и выше).
             Также соответствует максимальному устанавливаемому количеству соединений с БД-источником.
         -->
        <reader-pool size="4"/>
        <!-- Количество потоков записи (целое число от 1 и выше).
             Также соответствует максимальному количеству сессий с БД-получателем.
             Если не указано, используется значение reader-pool size.
         -->
        <writer-pool size="4"/>
        <!-- Максимальное количество батчей в очереди между потоками чтения и записи.
             Если не указано, используется значение reader-pool size.
         -->
    ...
    <table-options name="default">
        ...
        <!-- Начальное число партиций YDB-таблицы (PARTITION_AT_KEYS в DDL).
             auto - скопировать число и границы партиций из источника. Если у источника
                    нет партиций или их диапазоны по первой колонке ключа перекрываются,
                    разбиение пропускается, YDB сам распределит по нагрузке.
             N    - целое >= 2, создать N партиций разбиением на равные интервалы по
                    диапазону первой колонки ключа.
             none - не задавать начальное разбиение, YDB управляет партициями сам
                    через AUTO_PARTITIONING_BY_SIZE/LOAD.
    ...
    <table-ref options="default">
        ...
        <!-- Делит таблицу на split-count интервалов по колонке split-by для
             ...
             Каждый из тегов split-count, split-from, split-to принимает
             число (или конкретную дату для границ), либо auto.
             При auto:
                split-count наследует значение ydb-partition-count,
                split-from и split-to берутся как MIN/MAX от источника.
         -->
